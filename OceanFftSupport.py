...
def generate_hlsl_horner_expression(coeffs, var, use_mad=True):
    """
    Generates a string representing the evaluation of the polynomial with the given coefficients using Horner's metho
    in HLSL syntax. Recommended to leave use_mad=True to, as it can be faster and more accurate than separate multiply
    and add operations. Fun fact! In HLSL, mad() is only for floats... fma() is for doubles! :) :) :)
    ...
    Returns:
        str: A string representing the polynomial evaluation in HLSL syntax.
    """
    if use_mad:
...
def get_normalization_shader_function_str(
    ...
    print(f"float {name}NormalizationFactor(in float s)", file=str_buffer)
...
def write_shader_snippets_to_file(file_path : Path, filename : str, snippets : list):
    ...
    with open(shader_file_path, 'w') as f:
        f.write("// Auto-generated shader code for directional spreading function normalization factors\n")
...
def write_exr_lut(output_directory : Path,
                  file_label : str,
                  data : np.ndarray,
                  lut_parameters : LUTParameters):
    """
    Writes the given LUT data to an EXR file in the specified output directory. The filename is constructed using the
    provided label as the base, with suffixes indicating resolution and precision (f16 or f32). 
    """
    if not Path(output_directory).exists():
        print(f"ERROR: Directory {output_directory} for generated LUTs does not exist. Exiting.")
        raise SystemExit(1)
    
    suffix = "f16" if lut_parameters.use_f16 else "f32"
    lut_filename = f"{file_label}_{lut_parameters.resolution}_{suffix}.exr"
    lut_output_path = output_directory / lut_filename

    if lut_output_path.exists():
        print(f"WARNING: Overwriting existing LUT file at {lut_output_path}")

    print(f"====== Writing EXR LUT to {lut_output_path} ======")
    h, w = data.shape
    header = OpenEXR.Header(w, h)
    data = np.asarray(data, dtype=np.float32)
    out_pixels = data.astype(np.float16 if lut_parameters.use_f16 else np.float32)
    channels = { 'R' : out_pixels }
    with OpenEXR.File(header, channels) as exr_file:
        lut_output_str = str(lut_output_path)
        exr_file.write(lut_output_str)

    print(f"====== Finished writing EXR LUT to {lut_output_path} ======\n")
...
def generate_hasselman_lut_sampling_function(beta_bounds : ValueBounds, swell_bounds : ValueBounds):
    ...
    """
    function_definition_str = "float SampleHasselmanLUT(in float beta, in float swell, in Texture2D normTexture, in SamplerState normSampler)"
...
if __name__ == "__main__":
    ...
    plugin_concat_path = Path(config_args["engine_path"] + "/" + config_args["plugin_path"])
    if not plugin_concat_path.exists():
        print(f"ERROR: Directory {plugin_concat_path} for generated plugin files does not exist. Exiting.")
        raise SystemExit(1)

    # Write out EXR LUT for Donnelan-Banner normalization factor based on the calculated Beta and Swell bounds
    q_bounds, lut_data = generate_db_normalization_lut(BetaBounds, S_SwellBounds, config_args["donnelan_banner_spreading"])
    lut_concat_path = plugin_concat_path / config_args["lut_output_path"]
    write_exr_lut(lut_concat_path, "donnelan_banner_normalization", lut_data, config_args["donnelan_banner_spreading"])


    shaders_concat_path = plugin_concat_path / config_args["shaders_path"]
    # Generate and print the polynomial fits for Hasselman and Mitsuyasu spreading functions
    ...
    write_shader_snippets_to_file(shaders_concat_path, "OceanFFT_Generated.hlsl", [mitsuyasu_str, hasselman_str, sampling_function_str])
