...
def write_cpp_lut(header_dir : Path, source_dir : Path, array_name : str, data : np.ndarray):
    """
    ...
    header_path = header_dir / f"{array_name}.h"
    source_path = source_dir / f"{array_name}.cpp"
    ...
    with open(source_path, 'w') as f:
        f.write("// Auto-generated LUT data -- do not edit\n")
        f.write("// Generated by OceanFftSupport.py\n")
        f.write(f"#include \"generated/{array_name}.h\"\n\n")
...
if __name__ == "__main__":
    ...
    project_root = Path(config_args["project_root"])
    shader_dir   = project_root / config_args["shaders_path"]
    header_dir   = project_root / config_args["cpp_header_path"]
    source_dir   = project_root / config_args["cpp_source_path"]

    for d in (shader_dir, header_dir, source_dir):
        if not d.exists():
            print(f"ERROR: Output directory {d} does not exist. Exiting.")
            raise SystemExit(1)
