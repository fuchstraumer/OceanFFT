 ...
 * 
 * I also use Session() structs as RAII wrappers to manage lifetimes of things like mapping/unmapping
 */
namespace velox
{
    ...
    namespace detail
    {
        ...
        inline void PrintStatusMessage(wgpu::MapAsyncStatus status, wgpu::StringView message)
        {
            ...
            std::println(stderr, "[velox][async] Buffer map unsuccessful | status {} | message {}",
                statusText, wgpuMessage);
#endif
        }
        ...
        inline void PrintStatusMessage(wgpu::CreatePipelineAsyncStatus status, wgpu::StringView message)
        {
            ...
            std::string_view statusText = magic_enum::enum_name(status);
            std::string_view wgpuMessage(message.data, message.length);
            std::println(stderr, "[velox][async] Pipeline create unsuccesful | status {} | message {}",
                statusText, wgpuMessage);
#endif
        }
    ...
    public:
        ...
        {
            detail::MappedPointerType<Mode> dataPtr = co_await BufferMapAwaitable<Mode>{ buffer, _size, _offset };
        ...
        {
            static_assert(std::is_standard_layout_v<T>, "T must be standard layout to interpret mapped data as span of T");
    ...
    template<typename T>
    concept PipelineType = std::is_same_v<T, wgpu::RenderPipeline> ||
                           std::is_same_v<T, wgpu::ComputePipeline>;
    ...
    private:
        ...
        using DescriptorType = std::conditional_t<std::is_same_v<Pipeline, wgpu::RenderPipeline>,
                                                  wgpu::RenderPipelineDescriptor, wgpu::ComputePipelineDescriptor>;
    ...
    public:
        ...
        constexpr PipelineAwaitable(wgpu::Device _device, DescriptorType _descriptor) noexcept :
            device{ _device }, descriptor{ std::forward<DescriptorType>(_descriptor) }
        {}
        ...
        void await_suspend(std::coroutine_handle<> handle)
        {
            ...
            {
                auto callback = [handle](wgpu::CreatePipelineAsyncStatus status, wgpu::RenderPipeline pipeline, wgpu::StringView message)
                {
